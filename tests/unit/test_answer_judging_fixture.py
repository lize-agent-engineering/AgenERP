...
⚠️ **上表的数字是历史叙述，逐字保留、不按今天的口径改写**（plan `2026-08-25-0850-1` 判据⑧）：
它记的是「我当时判成了什么」，不是「哪个数才对」。P1.0 第一轮逐格计数今天在本仓
**只有一处可复算的真相源** —— `docs/architecture/model-management.md` §12.3 的四列并置表
（派生器 `tests/unit/entry_gate_tally.py`，判据 `tests/unit/test_entry_gate_tally.py`）。
四列**并不一致**，孰为准归人裁定，本文件不选边。
