...
- **集子之外没有能区分「判断」与「匹配」的证据。** `controls.json` 里的 H5（截断）
  **只证明判定器不会把明显截断的输入判成 `correct`** —— 独立评审实测指出，
  plan §6 H1c(b) 那个两行规则里的 `len<300 → truncated` **原样通过 H5**（179 < 300），
  即 **H5 能被一个公认不做判断的假实现通过**。
  ⚠️ **本行是执行期改准的措辞**（plan §8 风险① 的执行期补记）：
  **不许**再写成「集子外证据只有 H5 一条（强度低）」—— 那句**高估了 H5**。
  另：**没有任何人读过 `run-01[:179]`**，去确认它落在 `truncated` / `incomplete` 边界的哪一侧。
