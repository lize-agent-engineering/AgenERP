...
## ⚠️ 2026-08-24 补记（Phase 3 收尾，就地改准两处）

1. **本目录是 14 份轨迹，不是 12 份**：`run-09` / `run-10` 两次 token 超限、未产出答案，
   按 plan §2 记 `无效` 并**补跑** `run-13` / `run-14`。两份无效轨迹原样留下，未删。
   有效运行 12 次 = 4 格 × 3 次，符合 plan §2 的样本要求。
2. **上一节「做到的是先只看答案文本逐条打勾、再看配置」这句，本轮没有做到**：
   判定者在打勾之前已经看过 `run_id → 模型 / 门禁` 的汇总表。
   判定表 [`verdicts.md`](./verdicts.md) 开头逐字记了这件事，按此折价。
