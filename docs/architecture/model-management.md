...
| `kimi-k3` | `tool_calling` | 是 | **2026-08-27 实测**（独立评测集 60 条选型）：给了工具定义后真发起 `meta_fields({"doctype": "Sales Order"})` ⇒ `tool_calling`；回包 `reasoning_tokens = 71` ⇒ 计 reasoning。⚠️ `long_context` / `reasoning` / `multi_hop` **那次探针判不了，故不声明** ——今天只够 `permission` / `explain` 两档 |
