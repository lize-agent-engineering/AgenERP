...
`qwen3.6-plus` 在**本项目自己的两跳题**上，按人侧独立复核的逐格判定是
**无门禁 1/3、有门禁 0/3 —— 合计 1/6**。
⚠️ **这里有一处需要人裁定的算术分歧，不合并、不替人选**：`DECISIONS.md` D-11 的复验注记与
`STATE.md` §3 那条 P1.7 的 open 行都写作 **2/6**，而 `STATE.md` §2 复核表的两格相加是 **1/6**。
两处都是人写的，红线 3/5 不许 loop 改动其中任何一行，已随 P1.1 收口在 §3 追加 needs-human。
**本节按复核表的逐格数取 1/6**（D-16：以本项目的实测为准，而逐格判定就是那份实测）。
