...
#### 🔴 2026-08-27 · 路线 C：把「100% 完备」的分母换掉（人裁定，P2.2 已实测）

「让用户不跳 Desk」这个诉求是对的，但**若指「新前端覆盖全部功能」，那是 §3.2 已判走不通的路**。
人 2026-08-27 裁定走 **C**：**不改分子，缩分母** —— ③端用户每天其实只碰 3–5 个 DocType，
对这几个做到全覆盖，**这些人在日常路径上就永远不用跳**。兜底原则一字不动，只是不触发。

**在「车间工人」这一个角色上已实测成立**（`worker@hrd.example.com`，权限由站点上的
`Custom DocPerm` 强制，仅可读 `Work Order` / `Stock Entry` / `Item`）：

| | |
|---|---|
| 覆盖面 | 3 个可读表 + 12 张子表 = **15 张 / 317 个字段** |
| 动工前渲染器已覆盖 | 299/317 = **94.3%**，缺 **4 种 fieldtype、18 个字段** |
| 三个日常视图的落回 | **3 → 0** |
| 验收 | `pytest -m live tests/render -q` → **退出码 0，14 passed** |

⇒ **路线 C 的可达性不是推测，是算出来的**：要补的是 4 种，不是 §3.2 说的那 30 种。

⚠️ **「落回 = 0」不等于「落回画对了」。** P2.2 的变异验证第一轮撞到这一格：
把落回卡片整段删掉，12 条判据一条都没红 —— 因为落回被压到 0 之后，
**那条路在活体里一次都没被走过**。已补一个「真的会落回」的探针视图守它。

⚠️ **只对这一个角色成立。** 别的角色可读的 DocType 不同、缺的 fieldtype 也不同，**推广要另量**。

详见 plan [`2026-08-27-P2.2-…`](../plans/p2-views/2026-08-27-P2.2-renderer-role-complete.md)。
