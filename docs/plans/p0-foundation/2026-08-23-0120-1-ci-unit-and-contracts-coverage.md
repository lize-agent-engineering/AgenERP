...
- [x] **实验 ③（`tests/unit` 步骤有牙齿）**：推上述变异。
      **预测**：新 job `failure` 且**红在步骤 ①**（**不是** ②）；`gates-l1` 的预测值**照抄上一项的本机测量**。
      ...
      - Skill: `none`
- [x] **实验 ④（revert 必绿）**：`git revert` 实验 ③ → 11 个 job 全 `success`。这一次绿是落 `main` 的前置。
      - Skill: `none`
- [x] **Proof：revert 后的树与实验前逐字节相同**：`git diff <实验前 sha>..HEAD -- agenerp/` **无输出**。
      - Skill: `none`
- [x] **Proof：成本记账（本 Phase 只记本 Phase 已发生的那几次，不记还没发生的）。**
...
- [x] 实验 ① 的本机三条前置判据（exit 0 / exit 1 / exit 0 且三行不变）与**确切变异点**记在本 plan 内
- [x] 四条实验各有一个 run id 与结论，且每条的「预测」在本 plan 里位于「实测」之上（写作顺序即证据顺序）
- [x] 实验 ① 实测「红在步骤 ② 且其余 10 个 job 全 `success`」—— 若不成立，照实记并停机，**不得改写预测**
- [x] `git diff <实验前 sha>..HEAD -- agenerp/` 无输出
- [x] **本 Phase 结束前已发生的 5 次**运行的 CI 分钟数记在本 plan 内（落地与文档那几次归 Phase 4）
- [x] `docs/logs/` 更新
...
- 新 job `单测与契约测试（439 条）` `97073436302` → **`failure`**，
  ...
  —— **红在步骤 ②，与预测逐字一致**。日志逐字 `288 passed in 2.93s` 与 `6 failed, 145 passed in 0.19s`。
- **其余 10 个 job 全部 `success`**：`roadmap 引擎可解析` `97073436091` · `L1 快门禁` `97073436172` ·
  `循环联动冒烟` `97073436179` · `门禁未被改动` `97073436180` · `L2 全量 live 判定（19 条）` `97073436193` ·
  `预期红名单只能变短` `97073436208` · `L2 种子链（装载 + 站点侧对账）` `97073436212` ·
  `L2 慢门禁（零依赖启动）` `97073436231` · `判定器未被改动` `97073436242` · `主计划引用不断链` `97073436243`。
...
`预期红名单只能变短` `97073967359` · `L2 种子链（装载 + 站点侧对账）` `97073967398` ·
...
- 新 job `97074525545` → **`failure`**，步骤级逐字：`① 单测（tests/unit）` **`failure`** ·
  ...
  **同时这是 D2 那条 fail-fast 残余风险的第一次实证**：步骤 ② 的结论逐字就是 `skipped`，
  一次红确实只报了一半。日志逐字 `4 failed, 284 passed in 2.52s`。
- **`gates-l1` `97074525459` → `success`**，与「照抄本机测量」的预测一致（本机判定器 exit 0 且三行不变）。
- **独占性：本 plan 事先声明不预测也不宣称，实测到什么就记什么** —— 实测其余 10 个 job **全部 `success`**，
  含 `L2 全量 live 判定（19 条）` `97074525565` 与 `L2 种子链（装载 + 站点侧对账）` `97074525464`
  两条活站点链。**这条只是观察，不是本 plan 的预测命中**，也不得反推成
  「`diff()` 不在活站点链上」这类更强的结论。其余：`预期红名单只能变短` `97074525352` ·
...
**不构成「连续 3 轮 `GATE_VERIFY` fail」**。

**成本记账（到本 Phase 结束为止已发生的 5 次全量运行，落地与文档那几次归 Phase 4）**：

| run | 用途 | 墙钟 | job-分钟合计（11 个 job 逐个求和） |
|---|---|---|---|
| `32590196838` | Phase 2 PR 首绿 | 3 分 28 秒 | 10.8 |
| `32590487279` | 实验 ① | 3 分 34 秒 | 11.2 |
| `32590701768` | 实验 ② | 4 分 00 秒 | 11.8 |
| `32590923810` | 实验 ③ | 3 分 18 秒 | 10.9 |
| `32591113070` | 实验 ④ | 3 分 28 秒 | 11.2 |
| **合计** | **5 次** | **17 分 48 秒** | **约 55.9 job-分钟** |

⚠️ **计费口径照实说**：`gh api …/timing` 对这 5 个 run 逐个回 `billable.UBUNTU.total_ms = 0`
（**公开仓的 GitHub-hosted ubuntu runner 不计费**），所以上表的「job-分钟」是
**由每个 job 的 `startedAt`/`completedAt` 求和算出的实耗**，**不是 GitHub 账单数字**，两者不得混同。
新 job 在其中的占比很小（每次约 11–20 秒），**5 次里 46 分钟以上仍由三个 docker job 贡献**。
