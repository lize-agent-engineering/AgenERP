...
- [ ] `Explore` **E1 · 在隔离探针上跑通链路，不碰 `XM-LACE-1000`**。
      ...
      **每一步的原样输出记进 `docs/logs/`**，重点记站点算出的 `Bin.stock_value` 分层结果。
      - Skill: `none`
- [ ] `Explore` **E2 · 计价方法实读**：读回 `Stock Settings.valuation_method` 与 `Item.valuation_method`，
...
- [ ] `Decision` **D1 · 用哪条单据链复现 FIFO 分层**（依赖 E1/E2）。备选：
      ...
      **判定依据两条，按序**：① 站点自己算出 ¥6,450 且 E3 认定可查的指标全绿；
      ...
      - Skill: `none`
- [ ] `Decision` **D2 · 拟断言 ② 的达成率那一项怎么办**（依赖 E3 与 Current Baseline 第 6 条）。备选：
      ...
      **同时更新门禁提案**：拟断言 ② 的四项里有一项在当前口径下站点算不出来，这一事实必须写进提案，
      否则人采纳提案时会拿到一条注定红的门禁。
      - Skill: `none`
- [ ] `Decision` **D3 · 「站点算出来的数与期望对不上」的固定处置**（本 plan 最可能的失败模式，必须在动手前写死）。
      ...
      **同时被禁止的做法，逐条列名，不许挑一条做**：改 `model.py` 的数值常量 · 改 `checks.py` 的 `EXPECTED_*` ·
      ...
      - Skill: `none`
- [ ] `Proof`：E1–E4 的原样输出与 D1–D3 的结论按红线 5 追加进 `STATE.md` §2（含命令原文 + 退出码 + sha），
      并写进 `docs/logs/2026/08-22.md`。
      - Skill: `none`

Exit Criteria:

- [ ] E1 全程在**隔离探针**上完成；`docs/logs/` 有每一步原样输出，且**站点算出的 `stock_value` 已得到一个具体数**
- [ ] E2 的 `valuation_method` 实读值有留痕；E3 逐项给出读取路径或「查不出来」的明确结论；E4 给出幂等键结论
- [ ] D1 记录含「站点算了哪几段成本、哪几段是喂进去的」的逐字列举
- [ ] D2 有结论，且门禁提案已按结论更新（若采纳 (c)）
- [ ] D3 的固定处置与禁止清单逐字落在本 plan 内
- [ ] `docs/logs/` 更新，`STATE.md` §2 追加证据行
...
- [ ] `Add`：`SiteClient.submit_doc(doctype, name) -> dict` —— 把 `docstatus` 由 0 推到 1，
      ...
      **方法名含 `WRITE_VERBS` 里的 `submit`，必须同时登记进 `tests/unit/test_site_client.py` 的
      ...
      - Skill: `none`
- [ ] `Add`：`agenerp/seedsite.py` 的单据段 —— 按 D1 选定的链路装载并提交。
      **覆盖面逐条列名**：`Sales Order`（来自 `masters.py:95`）· `Work Order`（`masters.py` —— 若 D1 选 (b) 则显式排除并写明）·
      ...
      - Skill: `none`
- [ ] `Add`：CLI `python3 -m agenerp.seedsite --load-documents --site <site>` —— 幂等
      （已提交过的单据命中读回分支，不重复提交），失败即停退非 0，不留半装状态。
      **`--seed 42 --verify` 与 `--load-masters` 的既有行为逐字节不变。**
      - Skill: `none`
- [ ] `Add`：CLI `python3 -m agenerp.seedsite --verify-site --site <site>` —— **站点侧对账**。
      ...
      **不得在本模块里写任何新的期望字面量**（Current Baseline 第 3 条：`checks.py` 才是判官侧那份副本）。
      ...
      - Skill: `none`
- [ ] `Proof`（纯逻辑半）：`tests/unit/` 补单据映射与对账判定的单测（FakeTransport），
...
- [ ] 三个 CLI 面（`--load-masters` / `--load-documents` / `--verify-site`）各自行为落地并互不干扰
- [ ] `submit_doc` 已登记进 `WRITE_METHOD_ALLOWLIST`，守卫仍绿且仍有牙齿
- [ ] `--verify-site` 的期望值来自 `checks.EXPECTED_*`，有单测把这条结构约束钉住
- [ ] `python3 -m pytest tests/unit -q` → exit 0；`ruff check agenerp tests/unit tests/contracts` → exit 0
...
- [ ] `Proof` **R1 · 基线轮**：跑一次 Infrastructure 的干净站点循环 ①②，
      ...
      **`--load-documents` 原样再跑一次** → exit 0 且新建计数为 0（幂等的判据是第二跑的计数）。
      - Skill: `none`
- [ ] `Proof` **R2 · 承重判据**：`… python3 -m agenerp.seedsite --verify-site --site frontend` → **exit 0**，
      输出含**带出处标注**的站点实得 `actual_qty` / `stock_value` 与 `checks.EXPECTED_*` 的期望值。
      **若不等，走 Phase 1 的 D3 固定处置，一个字不许绕。**
      - Skill: `none`
- [ ] `Proof` **R3 · 变异验证，每条都从干净站点重来**。
      ...
      **每条都要记：改动内容 · 命令原文 · 退出码 · 站点读回的 `Bin` 原样输出 · 复原后的复跑退出码。**
      ...
      - Skill: `none`
- [ ] `Proof` **R4 · 计价条件留痕**：把 E2 实读的 `valuation_method` 与 R2 的实得值并排记进 `docs/logs/`，
      写清「¥6,450 是在**这个** `valuation_method` 下成立的」。**不写「§12.8 已闭合」**（Goals 4）。
      - Skill: `none`
- [ ] `Proof` **R5 · 不弄脏既有判据**：装载前（R1 第二步）与装载后各跑一次
...
- [ ] R2 在干净站点上 exit 0，输出含带出处标注的站点实得值与 `checks.EXPECTED_*` 期望值
- [ ] `--load-documents` 连跑两次均 exit 0，第二次新建计数为 0
- [ ] R3 三条变异各有「干净站点 → 变异 → 转红 → 点名 → 复原（含站点复位）→ 转绿」的完整留痕
- [ ] R4 的 `valuation_method` 与实得值并排留痕，且**未出现「§12.8 已闭合」这类陈述**
- [ ] 装载前后两次整目录 live 判定均 exit 0
- [ ] `docs/logs/` 更新，`STATE.md` §2 追加证据行
...
- [ ] `Fix`：更新 `docs/backlog/gate-proposal-seed-dataset.md` —— 逐条标注三条拟断言的现状：
      ...
      **`Status: proposed` 不改，不代人采纳。**
      - Skill: `none`
- [ ] `Add`：往 roadmap 工作项 7 处**追加**一条现状行（追加，不改写既有行）：
...
- [ ] §12.7 已改准且未出现「工作项 7 已有站点侧门禁」这类假陈述
- [ ] §12.8 补了本仓站点的实读事实，且**证据仓那条推断仍明确开着**
- [ ] 新增小节含单据链裁定与「站点算了哪几段成本」的逐字列举
- [ ] `project-context.md` 新增一行，四条口径齐备
- [ ] 门禁提案三条拟断言逐条标注现状，达成率那项的不可达原因已写明，`Status` 未改
- [ ] roadmap 只有追加：`git diff --numstat` 对该文件删除列为 `0`
- [ ] `docs/logs/` 更新
...
- [ ] relevant docs are aligned（§12.7 / §12.8 / 新增小节 / `project-context.md` / 门禁提案 / roadmap）
- [ ] verification has run：`python3 -m pytest tests/unit -q` · `ruff check agenerp tests/unit tests/contracts` ·
...
- [ ] **`tests/gates/**` 与 `tools/gates/expected-red.txt` 一个字节未改**
- [ ] **三份期望值副本一个未改**：`git diff` 对 `agenerp/seed/model.py` 的数值常量段、
      `agenerp/seed/checks.py` 的 `EXPECTED_*` 段、`tests/gates/test_seed_dataset_absurdity.py`
      **均无输出**；且 `--verify-site` 里**没有新的期望值字面量**（结构单测已钉住）
- [ ] **派生量未被当作装载输入**：`BACKLOG_QTY` / `BACKLOG_VALUE` / `COGS_VALUE` / `GROSS_PROFIT`
      在 `agenerp/seedsite.py` 里**零命中**（`grep` 可判）
- [ ] **单位成本类派生量的处置是显式的，不是默认放行**（评审第 2 轮补）：
...
- Auditor / Agent: <独立子代理，fresh session>
