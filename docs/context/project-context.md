...
| 种子主数据装载             | `AGENERP_HTTP_PORT=18080 docker compose -f docker-compose.yml up -d --wait --wait-timeout 300` **先起栈**，再跑 `AGENERP_HTTP_PORT=18080 AGENERP_SITE=frontend AGENERP_SITE_URL=http://127.0.0.1:18080 AGENERP_ADMIN_PASSWORD=admin python3 -m agenerp.seedsite --load-masters --site frontend`（2026-08-22 由 plan `2026-08-22-2107-1` 实测：从 `docker compose … down -v` 冷起的空站点起，**第一跑 exit 0 `合计：新建 40 / 已存在 0`，原样第二跑 exit 0 `合计：新建 0 / 已存在 40`**）。**四条口径必须一起看**：① **端口 18080** —— 8080 被本机另一套常驻 ERPNext 栈占着；② **必须先起栈** —— 装载器直连 REST，栈没起时红在 connection refused；③ **env 三个变量必须由命令给**（`AGENERP_SITE` / `AGENERP_SITE_URL` / `AGENERP_ADMIN_PASSWORD`），产品代码不内置口令默认值，缺凭据时报错并指名变量；④ ⚠️ **它不在 `missions/p0-foundation.json` 的 `commands.test` 里**，`GATE_VERIFY` 复跑不到它 —— 与 Contract tests / Seed dataset 两行同样的处理，理由相同（`missions/**` 是角色 B 禁区，要补由人做）；代偿控制是 `tests/unit` 单测 + 装载前后两次整目录 live 判定 + 独立关闭审计，见 `docs/architecture/module-boundaries.md` §12.9。**幂等判据是第二跑的「新建 0」，不是「没报错」**；**装了就留在站点上，没有代码级 teardown**，复位只能 `down -v` 冷起或人工 `bench restore`（§12.9） |
| 种子数据站点侧对账（单据装载 + 站点自己算出 1,010 米 / ¥6,450） | **干净站点循环是前置，四步缺一不可**：① `AGENERP_HTTP_PORT=18080 docker compose -f docker-compose.yml down -v` → ② `AGENERP_HTTP_PORT=18080 docker compose -f docker-compose.yml up -d --wait --wait-timeout 300` → ③ 上一行的 `--load-masters` → ④ `AGENERP_HTTP_PORT=18080 AGENERP_SITE=frontend AGENERP_SITE_URL=http://127.0.0.1:18080 AGENERP_ADMIN_PASSWORD=admin python3 -m agenerp.seedsite --load-documents --site frontend`；然后跑对账 `AGENERP_HTTP_PORT=18080 AGENERP_SITE=frontend AGENERP_SITE_URL=http://127.0.0.1:18080 AGENERP_ADMIN_PASSWORD=admin python3 -m agenerp.seedsite --verify-site --site frontend`（2026-08-22 由 plan `2026-08-22-2107-2` 实测：`--load-documents` **第一跑 exit 0 `合计：新建 17 / 已存在 0 / 提交 11`，原样第二跑 exit 0 `合计：新建 0 / 已存在 17 / 提交 0`**；`--verify-site` → **exit 0，9 项全过**，逐字 `✅ Bin(XM-LACE-1000, XM 成品仓 - XM).stock_value = 6450.00 / expected = 6450.00（出处：agenerp.seed.checks.EXPECTED_BACKLOG_VALUE）`）。**四条口径必须一起看**：① **端口 18080** —— 8080 被本机另一套常驻 ERPNext 栈占着；② ⚠️ **干净站点循环是强制前置，不是建议** —— 站点没有单据级撤销手段，在已装载的站点上再跑一次业务链会让 `Bin.actual_qty` 变成 2020 而不是 1010，**承重判据按构造不可达**；`down -v` **丢掉整站数据**，首次冷起前先 `docker compose exec -T backend bench --site frontend backup`；③ **env 三个变量必须由命令给**（`AGENERP_SITE` / `AGENERP_SITE_URL` / `AGENERP_ADMIN_PASSWORD`），产品代码不内置口令默认值；④ ⚠️ **它不在 `missions/p0-foundation.json` 的 `commands.test` 里**，`GATE_VERIFY` 复跑不到它 —— 与 Contract tests / Seed dataset / 种子主数据装载三行同样的处理，理由相同（`missions/**` 是角色 B 禁区）；代偿控制是 `tests/unit` **288 条**（⚠️ **2026-08-23 就地改准**：此前写 `283`，已被 plan `2026-08-22-2325-1` 新增的 5 条判据推翻；口径是 `python3 -m pytest tests/unit -q` 的实测通过数） + 三条变异验证 + 幂等第二跑计数 + 装载前后两次整目录 live 判定 + 独立关闭审计，见 `docs/architecture/module-boundaries.md` §12.10。**⚠️ 它是一条 CLI 退出码，不是门禁**：站点侧那三条 L2 断言仍是提案文本（`docs/backlog/gate-proposal-seed-dataset.md`，`Status: proposed`，采纳者是人）。**⚠️ 拟断言 ② 的「销售订单达成率 100%」不在这 9 项里**：它依赖本仓虚构的 `Loss Review` DocType，站点实测算出 `per_delivered = 99.0`，当前口径下算不出来（§12.10）。**⚠️ 两张发票的 `Overdue` 依赖真实时钟**（今天 > `due_date`），不是结构性成立 |
