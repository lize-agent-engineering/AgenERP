...
- 2. **schema 检索可用性**（P2.0R 🔴，头号风险）: `todo` —— 验收 **Top-5 ≥ 90%**。
  **四轮实测，最好 70.0%，仍差 20 个点。** 且验收判据 `tests/gates/test_schema_retrieval_recall.py`
  **尚未落地**（红线 1，只能由人建；草稿在 `tools/experiments/p2_schema_retrieval/GATE-DRAFT-*.txt`）。
  ...
  🔴 **第 5 轮把「还差什么」算清楚了**：合成 ≈ 选单据准确率 × oracle，
  **实测偏差只有 0.8 个点** ⇒ 要到 90%，字段层保持 92.5% 时单据消歧得做到 **97.5%**。
  ⚠️ **而 oracle 只有 92.5%，余量 2.5 点 —— 光把单据选到 100% 都不够富余。**
  ⇒ **下一轮必须同时攻字段层那 7.5%**（判定表事先写死的那一格）。
