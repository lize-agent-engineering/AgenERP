...
- 3. 模型路由 v0：OpenAI 兼容 adapter + 能力声明按任务分档（P1.1）: `done`（2026-08-24，sha `5a0f87a`；plan `docs/plans/p1-insight/2026-08-24-1457-1-model-routing-v0.md`，独立收口审计见其 `## Closure`。`agenerp/routing/` 五个模块 + `tests/routing` **132 passed, 1 skipped**；分档表落 `docs/architecture/model-management.md` **§12.5**（新增落点节），§12.3 那处「或由更强的循环门禁补偿」按 P1.0 **人侧独立复核**的判定改准。活端点冒烟跑过一次：`usage={'prompt':15,'completion':194,'reasoning':188}`，与开工前写死的预期 `reasoning > 0`（D-11）吻合。⚠️ **verification scope limited**：`tests/routing` 既不在 `commands.test` 也不在任何 CI job 里，与 `tests/tools` 同形态，**不得读成 CI 已覆盖**；已随收口在 STATE §3 追加 needs-human。⚠️ 另有两条 needs-human：`qwen3.6-plus` 的 `multi_hop` 声明建立在本项目 2/6 的实测上；本文件工作项 2 的逐格计数与下方 P1.7 警示表不一致，loop 未代改）
  ...
  **◆ 2026-08-26 第 2 个 plan 收口记录（本行原有文字照实保留，一个字未改写；⚠️ 行首那个 `done` 状态词一个字未动）**：plan `docs/plans/p1-insight/2026-08-26-2101-1-routing-guard-registration-drift.md` → `completed`。**表规 3 的 1–2 个 plan 预算就此用满（2/2）。** 交付面是**登记面**，不是新功能：`docs/architecture/model-management.md` §12.5 那段逐字「今天**没有任何判据拦得住这条路**」**已就地改准** —— 处置早在 2026-08-25 就由人 `d18c05c` 落地（`tests/gates/test_agent_seam_stays_swappable.py::test_chat_adapter_is_only_constructed_inside_routing`，至今 `2 passed`），**登记文字一个字没跟上**，`STATE.md:751` 那条 `[open] 2026-08-24T08:12Z` 因此被起草步反复当成活缺口读了四轮。同段第二句「真正的闸要等 P1.4 落地、有了唯一的调用入口」也已可判：**P1.4 已 `done`，而唯一入口没有出现**（`route()` 今天两个产品调用点：`agenerp/explain/loop.py:664` · `agenerp/judging/judge.py:73`）⇒ 照实写「这条预期没兑现」。新增 `<!-- machine-read: routing-guards -->` 表逐行写实「判据文件 · 判据函数名 · 盖住的形态 · **明确不盖的形态** · 实测日期 + 证据路径」，并由新判据 `tests/routing/test_routing_guard_registration.py` 钉住它与仓里真实判据**双向同构**（删行 / 改函数名 / 改文件路径 / 整表删空 / 证据路径失效，五种都红）。覆盖边界**全部由变异实测判定，不由读源码判定**（M1–M6 + L1/L2 八条，逐条预测在前、复原后 `sha256` 逐字节比对，证据 `docs/evidence/p1-routing-guard-registration/`）：盖住的只有「`agenerp/**` 里以裸名 `ChatAdapter(...)` 直接构造」一种；**盖不住四种** —— 别名导入 · 属性式构造 · `agenerp/` 之外的调用方（**今天就有实例** `tools/experiments/p1_insight_live/run.py:159`，打真端点）· **判据自身无存活守卫**（类名一改就永久静默绿）。四种都要改 `tests/gates/**` 才能堵（红线 1），**已登记交人，不假装堵上了**。

  ⚠️ **本记录上方那一行里的「⚠️ verification scope limited：`tests/routing` 既不在 `commands.test` 也不在任何 CI job 里」今天只对了一半，两半分开说**：**（一）`commands.test` 那一半仍真** —— `missions/p1-insight.json:16` 逐字仍只有 `python3 tools/gates/check_expected_red.py && python3 -m pytest tests/unit -q`，`tests/routing` **确实不在 `GATE_VERIFY` 的判定面上**，⇒ **`GATE_VERIFY` 退 0 不等于本次新增的那条判据被跑过**（`missions/**` 在红线内，loop 无权改，已登记交人）。**（二）「任何 CI job」那一半已假** —— `.github/workflows/gates.yml:617-618` 已有步骤 ④ 逐字 `python3 -m pytest tests/routing -q`，且 `:631` 的 `COVERED` 集合已含 `routing` ⇒ **`tests/routing` 早已进 CI**，落在该目录的新判据自动被 CI 跑到，**一行 workflow 都不用改**（红线 2 未触碰）。**本条按仓内惯例在原行下追加，原句一个字未改写。**
...
- 4. 上下文层 v0：即时上下文注入 + 会话落 DocType（P1.2）: `done`（2026-08-24；plan `docs/plans/p1-insight/2026-08-24-1457-2-context-layer-v0.md`。`agenerp/context/` 三个模块 + 会话 DocType **声明** + `tests/context` **53 passed**（WBS §4 P1.2 的验收原文 `pytest tests/context -q` 退 0；首次收口时 51 条，补 M9 判据后 53 条）。落点节 `module-boundaries.md` **§7.7**（新增），`context-and-memory.md` §8.2 的 ① / ② 两行补上落点指针（③ / ④ 一个字未动）。变异自查 **M1–M8 八个全部被打红**，无一需要就地补断言；**独立关闭审计**（sha `3337d69`）另出 A1–A5 五个变异，A5 打出一处判据缺口（② 档只有常量声明断言、没有行为断言），已回 EXECUTE 补上正向 + 反测两条并登记为 **M9**，复跑确认 M9 被打红，判据 **51 → 53 passed**。⚠️ **只做 ① 即时与 ② 会话两层**，③ 记忆 / ④ 检索是起草期显式定界的 Non-Goals，重开事件见 plan §9。⚠️ **会话在活站点上尚未建表** —— 那是风险档 L3 强制人批的动作，本 plan 未发出任何 DDL；已随收口在 STATE §3 追加 needs-human（含手工回滚命令原文）。**`closure audit was independent` 已勾** —— 独立关闭审计已于 sha `3337d69` 补做，结论 `issues`（见 plan §12.8）；⚠️ 但 **A5 的补齐（M9 两条断言）做在那轮审计之后、只经执行者自查**，不得读作「补齐也被独立复核过」。⚠️ **verification scope limited**：`tests/context` 在**首次**收口那一刻既不在 `commands.test` 也不在任何 CI job 里；此后由另一条工作线的 `b0ad632` 接进 CI 的 `unit-and-contracts`，本轮的 53 条会被 CI 复跑，**但那笔改动不属本 plan**。**◆ 2026-08-26 第 2 个 plan 记录（本行原有文字照实保留，唯一的字符级改动是行尾那个 `）` 被右移到本段之后）**：plan `docs/plans/p1-insight/2026-08-26-2213-1-ci-coverage-registration-drift.md` → `completed`（三个 Phase 全 `completed`）。**表规 3 的 1–2 个 plan 预算就此用满（2/2）。** 落地内容：**§7.7 那句「`GATE_VERIFY` 与 CI 都复跑不到本层的主判据」逐字与仓库相反** —— `gates.yml:620-621` 第 ⑤ 步逐字跑 `tests/context`、`:682` 的 ruff 参数含它，人早在 `b0ad632`（2026-08-24）就接进去了。**仍然成立的那一半没被一并改掉**：`missions/p1-insight.json:16` 的 `commands.test` 至今只有 `tests/unit` ⇒ **`GATE_VERIFY` 确实复跑不到**。落点 `module-boundaries.md` **新增 §7.26**（`<!-- machine-read: ci-coverage -->` 九行六列表 = 「哪些测试目录被谁复跑得到」在本仓的**单一真相源**），判据 `tests/unit/test_ci_coverage_registration.py`（七条断言、八个 `def test_`，双向同构 + 覆盖「步骤被 `if:` / `continue-on-error:` 悄悄关掉」），证据 `docs/evidence/p1-ci-coverage-registration/`。**同形态的其余各处按两种口径分开处置**：断言性散文**就地删被证伪的从句 + 加指针**（§7.7 · §12.5 七行 · §7.23.6 五条 · `project-context.md:52`）；带时点的账本 / 交付记录 / 探针快照**一个字不改写、只加时点限定 + 指针**（`:4270` · `system-baseline.md` §14.7 / §14.10 · `roadmap:41` · `roadmap:108-109` · `tools-dir-has-no-static-check-coverage.md`）。**变异自查 N1–N10 全部在 `/tmp` 整仓副本上施加、活仓零改动，十条与预测吻合**（含两条 must-stay-green 真的保持绿）；⚠️ **N3 首跑不符，照实记**：解析器只用了成对标记的起始那一半 ⇒ 表删空后越界读到下一张表、存活守卫从未被触发，当场补「在闭合标记处截断」（`a52a5d7`）并全表复跑。verification：`check_expected_red.py` → **exit 0**（`门禁 29 项：预期红 0，绿 29，跳过 0`，本 plan 不新增门禁）· `pytest tests/unit tests/tools -q` → **exit 0**（`928 passed, 29 skipped`，基线 `920` 只增不减）· `pytest tests/contracts tests/routing tests/context -q` → **exit 0**（`386 passed, 1 skipped`）· `ruff check …` → **exit 0**。⚠️ **verification scope limited**：未跑整仓 `pytest tests -q -m "not live"`（已知基线即红）· 未起 docker 栈 · 未跑任何 `-m live` · **未经 CI 服务端复跑**。⚠️ **归属取舍照实记**：本 plan 归工作项 4，却动了 §12.5（工作项 3）与 §7.23.6（工作项 11）的落点节；两轮独立评审对此给出过**相反判读**，本稿取第 2 轮的读法（表规 3 计 plan 数、不计文档小节），**人若不认同，`git revert` 掉 §12.5 那一处 hunk 即可**，全部经过与推翻方式逐字写在 `STATE.md` §3。⚠️ **独立收口审计由收口审计步执行，本轮执行者不代跑、不代批、不预填结论。**）
...
- 11. **Desk 侧边栏**（⌘K，调 P1.8a 的面）（P1.8b）: `done`
  - **◆ 第 2 个 plan 已交付并收口（2026-08-26）：`2026-08-25-1743-1-desk-sidebar-cmdk-and-live-ui-gate.md` → `completed`（表规 3 的预算此后 `2/2` 满）。**
    ...
    **本仓第一次真浏览器侧实证**：浏览器把 `HttpOnly` 的 `sid` 带到了 `/agenerp/explain`（直接读那次请求的 `Cookie` 头，morsel 名 `['full_name','sid','system_user','user_id','user_image']`，值长 56 字符），而同页 `document.cookie` 里**没有** `sid`。
    ...
    ⚠️ **`tests/ui/` 在 CI 上今天零覆盖，六件交接全部落在 `.github/workflows/**`（红线 2）** —— `COVERED` 少一个 `ui`（下次推送时 `unit-and-contracts` 第 ⑦ 步必红，**那正是那条守卫被写出来的目的**）· `gates-l2-live` 没有跑它的 step（**光加 `COVERED` 不会让它跑起来一次**）· `lint` job 的 ruff 参数不含 `tests/ui`。**装 chromium 的三个方案 + 实测墙钟/体积已一并交回，选哪个归人。****◆ 2026-08-26 追加（plan `2026-08-26-2213-1`，纯指针，本行已有文字一个字未改）**：本行的 CI 覆盖面登记**今天已过期**；当期真值**以 `docs/architecture/module-boundaries.md` §7.26 的登记表为准**，证据 `docs/evidence/p1-ci-coverage-registration/README.md`。
    ⚠️ **verification scope limited**：整仓 `pytest tests -q -m "not live"` **已跑但未绿**（**基线即红** —— 对照实跑把本 plan 新增的两份挪开，同一条命令 `5 failed, 1308 passed, 12 errors`）· **未经 CI 服务端复跑**（`tests/ui/` 不在任何 job 作用域里，**本门禁在 CI 上的行为零数据**）· **独立收口审计由执行者自己复跑，该 gate 留白**。**◆ 2026-08-26 追加（plan `2026-08-26-2213-1`，纯指针，本行已有文字一个字未改）**：括号里「`tests/ui/` 不在任何 job 作用域里、本门禁在 CI 上零数据」那半句**今天已过期**；当期真值见 `docs/architecture/module-boundaries.md` §7.26。
