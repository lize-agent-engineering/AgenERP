...
- 4. 上下文层 v0：即时上下文注入 + 会话落 DocType（P1.2）: `done`（2026-08-24；plan `docs/plans/p1-insight/2026-08-24-1457-2-context-layer-v0.md`。`agenerp/context/` 三个模块 + 会话 DocType **声明** + `tests/context` **51 passed**（WBS §4 P1.2 的验收原文 `pytest tests/context -q` 退 0）。落点节 `module-boundaries.md` **§7.7**（新增），`context-and-memory.md` §8.2 的 ① / ② 两行补上落点指针（③ / ④ 一个字未动）。变异自查 **M1–M8 八个全部被打红**，无一需要就地补断言。⚠️ **只做 ① 即时与 ② 会话两层**，③ 记忆 / ④ 检索是起草期显式定界的 Non-Goals，重开事件见 plan §9。⚠️ **会话在活站点上尚未建表** —— 那是风险档 L3 强制人批的动作，本 plan 未发出任何 DDL；已随收口在 STATE §3 追加 needs-human（含手工回滚命令原文）。⚠️ **`closure audit was independent` 这条 Closure Gate 未勾** —— 本轮未做独立关闭审计，收口强度低于工作项 3，待人安排。⚠️ **verification scope limited**：`tests/context` 在收口那一刻既不在 `commands.test` 也不在任何 CI job 里，**不得读成 CI 已覆盖**；人已另在工作树上把它接进 `gates.yml`，那笔改动不属本 plan）
