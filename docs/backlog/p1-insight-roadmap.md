...
- 10. **解释服务的 HTTP 面**（P1.8a，见 D-19）: `done` —— ⚠️ **本行原有文字全部照实保留、一个字未改写**（追加式账本）；**唯一的字符级改动是行首那个状态词 `todo` → `done`**，取代的理由与「交付了哪一半、没交付哪一半」逐条写在**本行末尾的「◆ 2026-08-25 第 2 个 plan 结清记录」**里。以下是第 1 个 plan 收口时写的原文 ——⚠️ **状态刻意仍是 `todo`，不是漏改**（表规 3 的预算是 1–2 个 plan，**第 1 个已收口，第 2 个未起草**）。本工作项的验收原文是 `pytest -m live tests/gates/test_explain_service_live.py` 退 0 + **零依赖启动门禁须仍绿**，两条**今天都还不成立**：门禁那份文件在红线 1 内、只能由人加载，而它要的**活栈 + nginx 同源反代**属第 2 个 plan。把它记成 `done` 会让引擎跳过第 2 个 plan，那才是漏。（**第 1 个 plan 已收口**：`docs/plans/p1-insight/2026-08-25-1159-1-explain-http-service.md`，落点节 `module-boundaries.md` **§7.20**。交付 `agenerp/serve/` 三个模块 —— `python3 -m agenerp.serve` 起标准库 `ThreadingHTTPServer`，端口读**新变量** `AGENERP_SERVE_PORT`（默认 `8330`，**不复用**站点端口 `AGENERP_HTTP_PORT`），两条端点 `GET /agenerp/health`（不认人、不碰 LLM、不打站点，恒 200）与 `POST /agenerp/explain`（认人）。**身份只从请求里的 `sid` 来**，`client_from_sid` → `frappe.auth.get_logged_user` → 用户名 → `explain(user=...)`；**零凭据回退**。**① 即时上下文的权限缺口按 `D-a-3` 的 (iii) 关**：请求体只收 `question` / `task_class` / `doctype` / `name` 四个键，字段表由服务端**用调用者自己的 `sid` 现取**（权限由 Frappe 判），`role` 取自 `sid` 解析出的人、`view` / `actions` 服务端写死，**请求体自带这五个键一律 400 而非静默忽略**。失败到状态码八格（401/403/400/503/502/404/405），**503 与 502 的分法是结构性的**（先显式取一次配置）。判据 `tests/unit/test_explain_service.py` **59 条**（十条判据族）+ 假件 `tests/unit/serve_fakes.py`。`python3 tools/gates/check_expected_red.py && python3 -m pytest tests/unit -q` → **exit 0**（`门禁 26 项：预期红 0，绿 26，跳过 0` · 697 → **756 passed**）；`pytest tests/contracts tests/tools tests/routing tests/context -q` → **exit 0**（`456 passed, 13 skipped`）；`ruff check ...` → **exit 0**；`git diff -- pyproject.toml` → **无输出**（**零新增第三方依赖**，H6）。变异自查 **M1–M11 逐条被打红，无一需要补断言**；其中 **M11**（`SiteClient(site, admin_password=credential_from_env(...))` 这条**绕开 `client_from_env` 的等价凭据回退**）打红 16 条。§6 的 **H1–H7 七格全部吻合**，预测原文一个字未改；**H3 由 `E-a-1` 的活站点只读探针实证**（`docs/analysis/2026-08-25-1159-explain-service-sid-probe.md`，八行实测，**零 `sid` 落盘**并做过反向自证）。**一处确认的引用漂移一并改直**：`agenerp/site.py:499` 与 `tests/unit/test_site_client_sid.py:301` 两处都指向 `test_explain_service.py` **判据⑩**，而服务面的凭据 AST 扫是**判据⑧** ⇒ 文件名与编号一起改。⚠️ **本 plan 不碰 `docker-compose.yml`、不碰 nginx、不向宿主发布任何端口，也不做 Desk 侧边栏**（Non-Goal 1 / 3，后者是工作项 11）。⚠️ **WBS §4 P1.8a 的 🔴 门禁未创建、也未声称已满足**（红线 1）—— 交付的是它的**断言体** `tests/unit/test_explain_service_body.py` 与交接说明（含**人要做的那一处收严**：skip → fail），由人在第 2 个 plan 落地的同一个提交里按路径加载；已在 STATE §3 追加 needs-human。⚠️ **`client_from_sid()` 在活站点上认得出人这件事，服务面这一侧仍未在活栈上跑过** —— 十条判据全部离线，活体那一半的自然归属是第 2 个 plan。⚠️ **`STATE.md` §3 `[open] 2026-08-25T00:35Z` 第 ① 项不因本 plan 落地而消失**：(iii) 关掉的是**本服务这个入口**上的缺口，`agenerp/context/immediate.py` 那一层「不查权限」的事实一个字没变，处置权仍在人。）
