...
- 1. P0 · 地基（无 Agent）: `done`

> **2026-08-23 P0 收口**：mission `p0-foundation` 九个工作项全 `done`，关闭判据逐条实跑全部 exit 0。
> 门禁：`live` 模式 **19 项全绿零红零跳过**，`default` 模式 **11 项全绿**，预期红名单 **0 条**。
> CI **14 个 job 全绿**（含三个真拉 ERPNext 栈的 live job，每次 push 到 main 都跑）。
> 产品代码 4,371 行 / 测试 6,302 行 / 175 个提交。**P0 的目标「把可验证做出来」已达成**：
> 此后任何一条能力宣称，都有一条可复跑、由退出码裁决的门禁顶着。
- 2. P1 · 解释与洞察（②端只读）: `done`

> **2026-08-26 P1 收口**：mission `p1-insight` **原定 12 个工作项全 `done`**，
> 另有 1 个交付后拆出的缺陷行（`3b`）也已 `done` ⇒ 13/13。
> 判定面 **29 项全绿零跳过**（本机）· **54 项全绿零跳过**（CI live）· 测试 22644 行。
> 阶段复盘 [`2026-08-26-CP9-P1-retrospective.md`](../audits/2026-08-26-CP9-P1-retrospective.md)：
> **AGE / mission-driver → 续用（以 loop 那次判据抽查为条件）· LoopX → 维持停用**。
>
> ⚠️ **收口时必须一并写下的一句**（复盘 §5，不放进成绩里）：
> **工作项 11 标 `done` 时验收命令确实退 0 —— 而那时 Agent 连一个归因问题都答不出来。**
> 交付后实测才补的五件（模型配置生效 / 失败说原因 / 答案面去 `or` /
> `doc.links` 跳 Single / 上限 25·32 → 40·50）**全部在 P1 验收范围之外**，
> 把归因类问题成功率从 **0/3 拉到 3/3**。
> ⇒ **P1 按它自己的判据完成了，而本阶段证明那套判据不够。**
>
> ⚠️ **本阶段出现过 1 次真实的门禁误放行**（答案面判据一个 `or` 让空答案照过，
> 挂了数日、**不是被任何判据发现的**）。CP1 曾把它列为头号风险、P0 复盘判过「= 0」——
> **该风险已实际发生，不再是理论风险**。补救已落地：RUNBOOK §7.2.1 把该判据从
> **事后统计**改为**抽查**（抽查者不得是作者）。
