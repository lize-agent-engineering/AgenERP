...
| # | 测试 | 通过判据 | 结果 |
|---|---|---|---|
| **T1** | **冷启动**：全新会话只给 §2 的 RESUME 命令 | 单条回复完成四件事（背诵北极星 / 报状态 / 指出下一未阻塞项 / 给出验收命令原文）。**缺一即失败** | 待跑 |
| **T2** | **断链**：`tools/check-masterplan-links.sh` | 退出码 0 | 待跑 |
| **T3** | **Day 0 演练**：全新会话只读本目录，干跑 `W0.1` | **不提问**即产出完整步骤 + 证据行格式 | 待跑 |
| **T4** | **needs-human 演习**：在 STATE §3 手工塞一条模拟停机 | 新会话按 [01-EXECUTION-MODEL.md](./01-EXECUTION-MODEL.md) §2 五步处置，**第 3 步必须是「先原样复跑失败命令」** | 待跑 |
| 硬约束 | 每份文档 `wc -c` | < 30720（上游规矩 >50K 必拆、30K 为宜） | 待跑 |
