...
- 2026-08-21T15:48Z · **P0.7** · `AGENERP_HTTP_PORT=18080 AGENERP_LIVE=1 AGENERP_ADMIN_PASSWORD=admin python3 -m pytest tests/gates/test_zero_dep_boot.py -q` → **exit 0**（`3 passed`，此前唯一仍红的 `::test_homepage_states_ai_disabled_instead_of_crashing` 转绿）· sha `6ac1005` · 下一项：工作项 8 两半均已落地，roadmap 保持 `planned`（名单未划），由人决定下一项
  · **交付**：plan `docs/plans/p0-foundation/2026-08-21-2220-2-homepage-ai-not-configured.md`（工作项 8 的第二个、也是最后一个 plan）交付**首页降级文案 + CI 真跑 L2 两半**。承载物取 `Website Settings.banner_html`，**不取**「建 Web Page + 改 `home_page`」——后者会把 `/` 从登录页上夺走，而本仓此刻唯一可用界面就是 ERPNext 原生桌面。落点是 compose 新增的一次性服务 `bootstrap-homepage`，跑仓内脚本 `tools/bootstrap/homepage_notice.py`（经**本仓第一个 bind mount**、只读、宿主侧路径字面写死）。口径与两条残余风险写进 `docs/architecture/system-baseline.md` §14.3。
  · **文案的硬约束不是文案偏好，是判据侧事实**：门禁那条断言**无条件**（`assert "AI 能力未配置" in resp.text`），而 `compose_stack` 起栈用的是**宿主环境**（`conftest.py` 的 `_compose()` 不传 `env=`）、compose 还会读仓根 `.env`。所以「AI 配上了就整段改写文案」会让门禁**在零代码改动的情况下、只因换台机器而变红**。定法：无论配没配，正文都逐字含「AI 能力未配置」，「已配置」分支只在它**之外**追加状态行——而 P0 阶段本仓不发起任何 LLM 调用，那句话在两个分支里都是真陈述。
  · **实测撞出一个编排层的坑，照实记**：一次性服务若**没有任何服务以 `service_completed_successfully` 依赖它**，`up -d --wait` 会把它的**正常退出**判成失败（第一次冷起 → **exit 1**，逐字 `container agenerp-bootstrap-homepage-1 exited (0)`，而引导本身是成功的）。`configurator` / `create-site` 不踩这个坑正因为它们各自被依赖着。处置：让 `frontend` 依赖它，顺带买到「nginx 对外服务时横幅一定已在」。
  · **本机冷起（`down -v` 连卷一起删）**：`AGENERP_HTTP_PORT=18080 docker compose up -d --wait --wait-timeout 300` → **exit 0**（62 秒）；`curl -H "Host: frontend" http://127.0.0.1:18080/` → **200**，正文含「AI 能力未配置」1 次，`<title>Login</title>` 仍在；再跑一次 `up -d --wait` → exit 0（5 秒），引导日志「已是目标内容，跳过」——**幂等成立**。
  · **变异验证（本门禁有牙齿）**：引导脚本文案改成不含该字符串 → **exit 1**，逐字红在「首页没有说明 AI 能力未配置」；改回（`shasum -a 256` 与变异前逐字相同）→ **exit 0**，`3 passed`。
  · **判据只加不放宽**：`tests/unit/test_compose_zero_dep.py` 补三条（一次性形状 / **AI 变量不得出现在任何 `healthcheck:` 块内** / **红线 7：引导逻辑不得交付运行时代码**，扫 `<script`、Jinja 定界符与两个运行时代码类 DocType 名），既有判据一条未动，三条各自做过变异验证。`python3 -m pytest tests/unit -q` → **exit 0**（189 → **192 passed**）。
  · **CI 真跑到了 L2（本行最重要的一条）**：`.github/workflows/gates.yml` **只新增一个 job** `gates-l2`。红线 2 自查：`git diff --stat .github/workflows/` → `1 file changed, 50 insertions(+)`，**零删除**——`on:` / `permissions:` 两个**整份工作流共享**的块与现有 6 个 job 一行未改，新 job 无 `continue-on-error`、无 job 级 `if:`（两处 `if: always()` 在 step 上，作用是「无论如何都拆栈」，是加严）。实跑：run id **`32499273158`**，sha `6ac1005`，`gh run view … --json conclusion` → **`success`**，七个 job 全绿；L2 job 用时 **3 分 37 秒**，起栈约 176 秒后所有服务 `Healthy`、三个一次性容器 `Exited`，门禁 **`3 passed in 2.68s`**，引导日志逐字「引导：首页横幅已写入（AI 能力状态）」——**在一台从来没有过这个站点的 runner 上，文案是引导步骤自己建出来的**。拆栈 `down -v` 把五个卷与网络全部 `Removed`。 ⚠️ **消掉的限定只有「零依赖启动只在本机验证过」这一条**：本次 CI 覆盖的是 `test_zero_dep_boot.py` 三条，`test_snapshot_diff_structured.py` 与 `test_customization_roundtrip_delete.py` **仍是本机独证**。
  · **版本差第一次拿到两侧对照数据**：runner 是 **Docker 28.0.4 + Compose v2.38.2**，本机是 **Docker 29.2.1 + Compose v5.0.2**，**同一份 compose 在两边表现一致**，compose 语法一个字节都没为 CI 改过（工作项 3 的交付面未被触及）。plan `1022-1` 登记的那条 watch-only residual 仍保持 watch-only：本次只覆盖 `test_zero_dep_boot.py` 三条，不是全部 L2。
  · **不请求划名单**：`tools/gates/expected-red.txt` **一行未动**（`git diff --numstat` 无输出），沿用人在本节 2026-08-21T11:20Z 的裁定。**代价照实记**：CI 的 L2 job 直接对 `pytest` 退出码判定、不跑判定器，因此那几条门禁**在 CI 上不受棘轮保护**；代偿控制是 `gates-untouched` job（断言就在 `tests/gates/**` 内，改它们要人工批准 trailer）。工作项 8 因此**保持 `planned`** 不置 `done`，与工作项 4/5/6/7 同一情形。
  · **两处确认的 owner-doc 漂移已就地改准，没有降级成 follow-up**（Minimum Rule 14）：`docs/context/project-context.md:48` 与 roadmap 工作项 8 对照行都写着「`compose_stack` fixture 仍抛 `NotImplementedError`（红线 1，等人处置，见 STATE §3）」——实测 `grep -c NotImplementedError tests/gates/conftest.py` → **0**，fixture 已由人在 `ede5440` 实现、阻塞已在 `3fed439` 关闭，§3 此刻一条 `[open]` 都没有。
  · **本行写在 §2 不写 §3**：§3 是 needs-human 队列，本 plan 的两个 `Decision` 全程由 loop 可解（名单口径落在候选 (i)，被排除的 (ii) 才需要人），没有任何要人拍板的事项。
  · **verification scope**：本机侧（Compose v5.0.2）与 CI 侧（Compose v2.38.2）**分开陈述**，两侧都拿到了退出码，见上。
  · **授权链（与本节此前几行同一处矛盾，照实引出、不擅自消解）**：`01-EXECUTION-MODEL.md` §1 的表写角色 B「不得手写 STATE」，而执行器人格 `tools/mission-driver/agents/build.claude.md` 逐字指示「拿不准就写进 `STATE.md` 的 needs-human 队列」；`AGENTS.md` 红线 5 允许**追加**证据行。本行按更高优先级那条执行，**只追加、不改写任何已有行**，也不为这处矛盾另开一行。
