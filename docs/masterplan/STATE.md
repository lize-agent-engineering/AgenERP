...
- 2026-08-21T15:48Z · **P0.7** · `AGENERP_HTTP_PORT=18080 AGENERP_LIVE=1 AGENERP_ADMIN_PASSWORD=admin python3 -m pytest tests/gates/test_zero_dep_boot.py -q` → **exit 0**（`3 passed`，此前唯一仍红的 `::test_homepage_states_ai_disabled_instead_of_crashing` 转绿）· sha `6ac1005` · 下一项：工作项 8 两半均已落地，roadmap 保持 `planned`（名单未划），由人决定下一项
  ...
  · **CI 真跑到了 L2（本行最重要的一条）**：`.github/workflows/gates.yml` **只新增一个 job** `gates-l2`。红线 2 自查：`git diff --stat .github/workflows/` → `1 file changed, 50 insertions(+)`，**零删除**——`on:` / `permissions:` 两个**整份工作流共享**的块与现有 6 个 job 一行未改，新 job 无 `continue-on-error`、无 job 级 `if:`（两处 `if: always()` 在 step 上，作用是「无论如何都拆栈」，是加严）。实跑：run id **`32499273158`**，sha `6ac1005`，`gh run view … --json conclusion` → **`success`**，七个 job 全绿；L2 job 用时 **3 分 37 秒**，起栈约 176 秒后所有服务 `Healthy`、三个一次性容器 `Exited`，门禁 **`3 passed in 2.68s`**，引导日志逐字「引导：首页横幅已写入（AI 能力状态）」——**在一台从来没有过这个站点的 runner 上，文案是引导步骤自己建出来的**。拆栈 `down -v` 把五个卷与网络全部 `Removed`。
