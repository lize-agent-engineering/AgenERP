...
| **下一个未阻塞工作项** | **W0.2 · 建仓库骨架（AGE）**（此字段**只填一个 ID**，不写「但实际前置是…」这类歧义——T1 实测：会让接手会话先做一次推理才敢动手） |
| 该项验收命令 | `ls docs/{architecture,design,backlog,context,testing,archive,plans}` 全部存在 |
