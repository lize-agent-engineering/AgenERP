...
#### 7.2.1 🔴 「门禁误放行」这一项**改成抽查，不再只统计**（人 2026-08-26 裁定，P1 复盘 B①）

**为什么改**：原来的形态是「本阶段门禁误放行次数 = 0」，P0 复盘据此判过 ✅。
**但 P1 产生了一个真实例**：答案面判据里一个 `or` 让空答案照过，
CI 每一次解释都失败、每一次回空答案，而它**一路绿着好几天** ——
**它不是被判据发现的，是人手动跑真解释才撞出来的**。

⇒ 事后统计**天然发现不了这种事**：没人发现，统计就是 0。

**新形态（每个阶段关口必做）**：

> 从本阶段**新增或改动过的绿判据**里**随机抽 3 条**，逐条回答一个问题：
> **「它测的，是不是它名字说的那件事？」**
> 抽查记录（抽了哪 3 条、各自的答案）落进当期复盘纪要。
> **抽到一条名不副实的，即为一次误放行**，按原规则处置。

⚠️ **抽查者不得是这些判据的作者。** 作者读自己的断言，读到的是当初的意图，
不是纸面上的字 —— P1 那次就是作者本人写的、作者本人核的、作者本人报的「全绿」。

#### 7.2.2 🔴 人侧改动也走独立复核（人 2026-08-26 裁定，P1 复盘 B③）

**为什么加**：今天的红线**只约束循环** —— 它改门禁要带 `Gates-Change-Approved-By:`、
要独立收口审计、要变异验证；**人侧代理改门禁只要自己写一句「已批准」**。

**这不是理论上的不对称。P1 期间人侧代理四次失误，三次是循环先发现的**：
① 从 `docker ps` 时间差推断出「六容器集体重启」（实际是分阶段启动）
② 新增测试里留下没用的 `import pytest`，CI ruff 当场红
③ `doc.links` 的守卫只加在非子表分支，**子表分支两处站点调用漏守**

**新规矩**：**人侧代理对 `tests/gates/**`、断言体、`agenerp/**` 的改动，
提交后必须登记进 `STATE` §3，由循环在下一轮独立复核**（口径同
`docs/skills/closure-audit-prompt.md`：不采信自报，逐条对活仓取证）。

⚠️ **它不阻塞人侧提交** —— 人可以先推，复核在后。它要的是**有人看过**，
不是**先批后动**。复核发现问题就登记，由人处置。

⚠️ **这一条约束的是写它的人自己。** 它之所以能立，正是因为上面那三条
是循环发现的 —— 证据在，不是自谦。
