...
| # | 红线 | 为什么 |
|---|---|---|
| 1 | **不得以任何方式修改 `tests/gates/**`** —— 包括改断言、加 skip/xfail、改 fixture 使其变松、删文件、改文件名 | 那是裁判。改裁判就没有裁判了。`git diff` 触及该路径 → **立即停机**，标 needs-human。**边界**：预期红名单 `tools/gates/expected-red.txt` 不在此列——它是账本不是裁判，测试转绿时应当在同一提交里划掉对应行（只能变短） |
...
4. **停机条件**（任一触发即停，宁可停不带病跑）：同一 plan 连续 3 轮 `GATE_VERIFY` fail｜`git diff` 触及 `tests/gates/**`｜单 mission 累计成本超阈值｜CI 连续 2 轮红｜**执行器认证失败**（重试无意义且只能由人修）。
